...
def print_tc():
    ...
    while enable_listening.get():
        ...
        if compare_timestamps(tcp, jam) < 0.1:
            send_mtc_signal(tcp)
...
def init_ltc_listener():
    ...
    t.start()
    device_info = p.get_device_info_by_index(micro_selectionne)
...
def send_mtc_signal(timecode_str):
    ...
    if not 0 <= hours < 24 or not 0 <= minutes < 60 or not 0 <= seconds < 60 or not 0 <= frames < 30:
        raise ValueError(
            "Invalid timecode values. Hours should be in the range 0-23, minutes 0-59, seconds 0-59, and frames 0-29 (depending on frame rate).")
...
def get_available_microphones():
    ...
    for i in range(num_devices):
        ...
        if device_info.get('maxInputChannels') > 0:
            microphones.append(device_info['name'].encode('windows-1251').decode('utf-8'))
